    ...
    const fetchData = async () => {
        try {
            ...
            setStats({
                ...
                totalDisbursements: data.total_disbursements || 0,
                todayPunches: data.today_punches || 0,
                weekPunches,
            });
        } catch (err) {
    ...
    const MiniBarChart = ({ data, maxValue, color }) => (
        <View style={styles.chartContainer}>
            <View style={styles.chartBars}>
                {data.map((item, index) => {
                    const barHeight = (item.punches / maxValue) * 100;
                    return (
                        <View key={index} style={styles.barWrapper}>
                            <Text style={styles.barValue}>{item.punches}</Text>
                            <View style={[styles.bar, { height: Math.max(barHeight, 4), backgroundColor: color }]} />
